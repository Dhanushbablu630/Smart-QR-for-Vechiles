...
import uuid
import os
from models import db, Vehicle, Message, ContactRequest, TripPhoto, Album, IncidentReport , Owner , MedicalInfo
...
def generate_qr(plate):
    with app.app_context():
        ...
        # Fixed hardcoded URL - use the generated one or make it configurable
...
def add_sample():
    ...
    sample = Vehicle(
        ...
        phone="9876543210",
        phone_shared=False,
        owner_id=existing_owner.id  # ✅ Assign owner_id
    )
    db.session.add(sample)
    db.session.commit()

    generate_qr(sample.license_plate)
    return "✅ Sample vehicle added!"

# Medical info route - FIXED
@app.route('/medical_info/<vehicle_id>', methods=['GET', 'POST'])
def medical_info(vehicle_id):
    ...
    medical_info_record = MedicalInfo.query.filter_by(vehicle_id=vehicle.id).first()
    
    if request.method == 'POST':
        if not medical_info_record:
            medical_info_record = MedicalInfo(vehicle_id=vehicle.id)
        
        # Update medical information
        medical_info_record.blood_type = request.form.get('blood_type')
        medical_info_record.medical_conditions = request.form.get('medical_conditions')
        medical_info_record.allergies = request.form.get('allergies')
        medical_info_record.emergency_medication = request.form.get('emergency_medication')
        medical_info_record.emergency_contact_name = request.form.get('emergency_contact_name')
        medical_info_record.emergency_contact_phone = request.form.get('emergency_contact_phone')
        medical_info_record.emergency_contact_relation = request.form.get('emergency_contact_relation')
        
        db.session.add(medical_info_record)
        db.session.commit()
        
        flash("✅ Medical information updated successfully!")
        return redirect(url_for('dashboard'))
    
    return render_template('medical_info.html', vehicle=vehicle, medical_info=medical_info_record)
    ...
    if request.method == "POST":
        ...
        if form_type == "send_message":
            ...
            if msg:
                ...
                send_telegram_notification(f"📨 New message from {sender_name} for {vehicle.license_plate}:\n{msg}")
            else:
                flash("⚠️ Message is empty.", "warning")
            ...
            db.session.commit()
            flash("📞 Contact request sent to owner!")
            send_telegram_notification(f"📞 Contact request from {requester} for {vehicle.license_plate}")
            ...
            if not message:
                flash("⚠️ Incident report must include a message.", "warning")
            ...
            db.session.commit()
            flash("✅ Incident reported to owner!")
            send_telegram_notification(f"⚠️ Incident report from {sender_name} for {vehicle.license_plate}:\n{message}")

        return redirect(url_for('vehicle_page', vehicle_id=vehicle_id))

    # Get medical info for display (if needed on vehicle page)
    medical_info_record = MedicalInfo.query.filter_by(vehicle_id=vehicle.id).first()
    
    return render_template("vehicle.html", vehicle=vehicle, medical_info=medical_info_record)
...
def dashboard():
    ...
    if not vehicle:
        flash("❌ Vehicle not found in DB.", "warning")
        return redirect(url_for("login"))
    ...
    # Get medical info for dashboard display
    medical_info_record = MedicalInfo.query.filter_by(vehicle_id=vehicle.id).first()
    ...
    if request.method == "POST":
        if "trip_image" in request.files:
            ...
            if photo:
                ...
                db.session.commit()
                flash("📌 Image pin toggled!")

        elif "share_phone" in request.form:
            vehicle.phone_shared = True
            db.session.commit()
            flash("📞 Phone number shared!")

        elif "hide_phone" in request.form:
            vehicle.phone_shared = False
            db.session.commit()
            flash("🔒 Phone number hidden!")

        return redirect(url_for('dashboard'))

    vehicle.trip_photos = TripPhoto.query.filter_by(vehicle_id=vehicle.id).all()

    return render_template("dashboard.html", vehicle=vehicle, albums=albums, incidents=incident_reports, medical_info=medical_info_record)
